...
# New clarity agents for team members
try:
...
class MasterSuperAgent:
    ...
    def __init__(self):
        ...
        self.agents: Dict[str, BaseAgent] = {
            ...
            # Clarity agents for team members
...
'''
    ...
    def run_strategic_cycle(self):
        ...
        # Run clarity agents for team members and push to Telegram
        if self.agents.get("EmployeeClarityAgent"):
            try:
                clarity = self.agents["EmployeeClarityAgent"].execute(Task(id="team_clarity", description="Generate personal clarity for all current team members", context={}))
                send_update("**Team Member Clarity Update**\n" + str(clarity.result.get("summary", "Your personal clarity report is ready in the portal. Check your action items.")) + "\n\nOpen portal → switch to Team Member view for exact pay, benefits, responsibilities and this week's moves.")
            except Exception as e:
                print("[Clarity push error]", e)

        try:
            summary = f"Master cycle: 50+ agents active. New spawns: {len(gaps)}. Builds for clients/PMs/devs/auditors/accountants/lawyers/owners. Clarity pushed for team members."
            send_update(summary)
        except:
            pass
